...
Class NoteExample Extends CGUI
{
	...
	treeNoteList_ItemSelected(Item)
	{
		...
		{
			if(!this.SelectedTreeItem.IsCategory) ;HasKey("Categories")) ;if selected item was a note, store its text
		...
		if(this.SelectedTreeItem.IsCategory) ;HasKey("Categories")) ;If new item is a category
		{
	...
	{
		if(this.SelectedTreeItem.IsCategory) ;HasKey("Categories"))
		{
...
#b::
tree := CGUI.GUIList[10].treeNoteList
msgbox break
return